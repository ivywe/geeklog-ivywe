...
var c=new Date,c=k+"?mode=editfile&path="+encodeURIComponent(b.Path)+"&config="+userconfig+"&time="+c.getMilliseconds();a.ajax({type:"GET",url:c,dataType:"json",async:!1,success:function(c){0==c.Code?(c='<form id="edit-form">'+('<textarea class="uk-textarea" id="edit-content" name="content">'+c.Content+"</textarea>"),c=c+'<input type="hidden" name="mode" value="savefile" />'+('<input type="hidden" name="path" value="'+b.Path+'" />'),c+='<button id="edit-cancel" class="edition" type="button">'+f.quit_editor+"</button>",
