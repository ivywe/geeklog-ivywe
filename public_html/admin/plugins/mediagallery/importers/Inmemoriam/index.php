...
// +--------------------------------------------------------------------------+
// | $Id:: index.php 5847 2010-04-09 13:16:50Z mevans0263                    $|
// | Inmemoriam gallery import script                                         |
// +--------------------------------------------------------------------------+
...
$sql = "SELECT * FROM {$_TABLES['plugins']} WHERE pi_name='inmemoriam'";
$result = DB_query($sql);
...
if ( $nRows > 0 ) {
    ...
    if ( $row['pi_enabled'] == 0 ) {
        ...
        $display .= '<br'.XHTML.'>' . $LANG_MG02['inmemoriam_not_installed'];
        $display .= COM_endBlock();
        $display = MG_createHTMLDocument($display, array('what' => 'menu', 'rightblock' => true));
        echo $display;
        exit;
    ...
    $display .= COM_startBlock($LANG_MG00['access_denied']);
    $display .= $LANG_MG02['inmemoriam_not_installed'];
    $display .= COM_endBlock();
    $display = MG_createHTMLDocument($display, array('what' => 'menu', 'rightblock' => true));
    echo $display;
    exit;
...
function MG_importSelectAlbums() {
    ...
    $T->set_var('form_action',$_CONF['site_admin_url'] . '/plugins/mediagallery/importers/Inmemoriam/index.php');
    $T->set_var(array(
...
function MG_buildImportAlbums( ) {
    global $mgAlbums, $INM_TABLES, $_TABLES, $_CONF, $_MG_CONF, $_USER;
    ...
    $sql = "SELECT * FROM {$INM_TABLES['albums']}";
    $result     = DB_query( $sql );
    ...
    for ($i=0; $i < $numRows; $i++ ) {
        ...
        $mgalbum = new mgAlbumg();
        $mgalbum->id = $I['id'];
        $mgalbum->galleryName = $I['name'];

        $mgalbum->title = $I['name'];
        if ( $I['pid'] == '' ) {
            $mgalbum->parent = 0;
        } else {
            $mgalbum->parent = $I['pid'];
        }

        $mgalbum->description   = $I['description'];
        $mgalbum->views         = $I['hits'];
        ...
        $mgalbum->max_filesize  = $I['max_filesize'];
        $mgalbum->max_image_height = $I['maxh'];
        $mgalbum->max_image_width  = $I['maxw'];
        $mgalbum->moderate      = $I['queue'];
        $mgalbum->member_uploads = $I['allow_submit'];
        $mgalbum->enable_comments = $I['enable_comments'];
        $mgalbum->enable_rating   = $I['enable_rating'];
...
function MG_importAlbums( $aid, $parent, $session_id=0 ) {
    ...
    for ($i=0; $i < $nrows; $i++ ) {
        ...
        if ( $_POST['gallery'][$x] == 1 ) {
            ...
            COM_errorLog("Media Gallery: Inmemoriam v1 Import processed " . $mgAlbums[$children[$i]]->title . " MGID: " . $mgAlbums[$children[$i]]->mgid . " Parent: " . $mgAlbums[$children[$i]]->mgparent);
...
function MG_importFiles( $album_id, $import_album_id, $session_id ) {
    ...
    global $mgAlbums, $INM_TABLES;

    $album_path = DB_getItem($INM_TABLES['albums'],'path','id=' . $import_album_id);

    $sql = "SELECT * FROM {$INM_TABLES['media']} WHERE aid=" . $import_album_id;
    $result = DB_query($sql);
    $nRows  = DB_numRows($result);
    for ($i=0; $i < $nRows; $i++ ) {
        $row = DB_fetchArray($result);
...
if (isset($_POST['mode']) ) {
    ...
    switch ($mode) {
        case 'convert' :
            $session_description = 'Inmemoriam v1 Import';
            $session_id = MG_beginSession('inmemoriamimport',$_MG_CONF['site_url'] . '/index.php',$session_description );
            MG_buildImportAlbums();
