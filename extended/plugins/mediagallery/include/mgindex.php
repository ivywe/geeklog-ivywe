...
function MG_index() {
	...
	*/
	...
	$themeStyle .= MG_getThemePublicJSandCSS($MG_albums[0]->skin); // Quicker than MG_getThemeCSS
	$themeStyle .= MG_getThemeCSS($MG_albums[0]->skin);
	...
	$admin_box  = '<form class="uk-form" name="adminbox" id="adminbox" action="' . $_MG_CONF['site_url'] . '/admin.php" method="get">' . LB;
	$admin_box .= '<div>' . LB;
	$admin_box .= '<select style="min-width:200px" onchange="javascript:forms[\'adminbox\'].submit();" name="mode">' . LB;
...
//                      $T->parse('AColumn', 'AlbumColumn',true);
	            ...
	            if ( isset($_MG_CONF['subalbum_select']) && $_MG_CONF['subalbum_select'] == 1  ) {
	                $subAlbumDisplay = '<form name="subalbums' . $MG_albums[$achild[$indexCounter]]->id . '" action="' . $_MG_CONF['site_url'] . '/album.php' . '" method="get" style="margin:0;padding:0"><div>';
	                            ...
	                            if ( $MG_albums[$SAchild]->access == 3 ) {
	                                ...
	                                if ( $_MG_CONF['subalbum_select'] == 1 ) {
	                                    if ( strlen( $MG_albums[$SAchild]->title ) > 50 ) {
	                                        $aTitle = COM_truncate( $MG_albums[$SAchild]->title, 50 ) . '...';
	                                    } else {
    ...
    $themeStyle .= $css;
    $title = $LANG_MG00['plugin'];
    $retval = $T->finish($T->get_var('output'));
    $retval = MG_createHTMLDocument($retval, array('pagetitle' => $title));
    return $retval;
}
