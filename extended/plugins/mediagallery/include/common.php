...
function MG_buildAlbumJumpbox(&$album, $album_id, $access=1, $hide=0)
{
    ...
    $retval .= $LANG_MG03['jump_to'] . ':&nbsp;<select class="uk-select uk-form-width-medium" name="aid" onchange="forms[\'jumpbox\'].submit()">' . LB;
...
function MG_buildAlbumBox(&$album, $album_id, $access=1, $hide=0, $type='upload')
{
    ...
    if (!empty($items)) {
        $retval  = '<select class="uk-select uk-form-width-medium" name="album_id" onchange="onAlbumChange()">' . LB;
        $retval .= $items;
        $retval .= '</select>' . LB;
    }
...
function MG_optionlist($info)
{
    ...
    $retval = '<select class="uk-select uk-form-width-medium" name="' . $info['name'] . '"'
