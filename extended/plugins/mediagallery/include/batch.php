...
function MG_batchDeleteMedia( $album_id, $actionURL = '' ) {
    ...
    for ($i=0; $i < $numItems; $i++) {
        ...
        foreach ($_MG_CONF['validExtensions'] as $ext ) {
            $tn = $_MG_CONF['path_mediaobjects'] . 'tn/'   . $row['media_filename'][0] .'/' . $row['media_filename'] . $ext;
            @unlink($tn);
            $tn_crop = MG_getThumbCropPath($tn);
            @unlink($tn_crop);
...
function MG_deleteChildAlbums( $album_id ){
    ...
    if ( count( $mediarow ) != 0 ) {
        for ( $i = 0; $i < count( $mediarow ); $i++ ) {
            ...
            if ( $row['count'] <= 1 ) {
                foreach ($_MG_CONF['validExtensions'] as $ext ) {
                    $tn = $_MG_CONF['path_mediaobjects'] . 'tn/' . $mediarow[$i]['media_filename'][0] .'/' . $mediarow[$i]['media_filename'] . $ext;
                    @unlink($tn);
                    $tn_crop = MG_getThumbCropPath($tn);
                    @unlink($tn_crop);
