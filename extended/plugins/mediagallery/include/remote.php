...
//        }
        ...
        list($rc,$msg) = MG_getRemote($URL, $mimeType, $album_id, $caption, $description,
            $keywords, $category, $attachedThumbnail, $thumbnail, $resolution_x, $resolution_y);
        $statusMsg .= $videoFile . " " . $msg . "<br" . XHTML . ">";
        if ($rc == true) {
            $successfull_upload++;
        }
    }
...
function MG_getRemote($URL, $mimeType, $album_id, $caption, $description, $keywords, $category, $attachedThumbnail, $thumbnail, $resolution_x, $resolution_y)
{
    ...
    if ($albumInfo['moderate'] == 1 && !$root_album->owner_id) { //  && !SEC_hasRights('mediagallery.create')) {
    ...
    $sql = "INSERT INTO " . $tableMediaAlbum . " (media_id, album_id, media_order) VALUES ('" . DB_escapeString($new_media_id) . "', " . intval($album_id) . ", $media_seq )";
    DB_query($sql);
...
//        MG_resetAlbumCover($albumInfo['album_id']);
    }
