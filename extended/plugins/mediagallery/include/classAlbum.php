...
//        CACHE_remove_instance('whatsnew');
    ...
    function showSelectTree( $depth ) {
        ...
        if ( $this->id != 0 && $this->access > 0 ) {
            ...
            if ( $depth == 0 ) {
                $retval .= "<div style=\"margin-left:" . $px . "px;\">"  . '<input type="checkbox" name="album[' . $this->id . ']" id="album_' . $this->id . '" value="1" ' . $block . '>&nbsp;&nbsp;' . strip_tags($this->title) . ' (' . COM_numberFormat($this->album_disk_usage/1024) . ' Kb)</div>' . LB;
            } else {
                if ( $level <= $depth ) {
                ...
                if ( !$this->hidden || ( $this->hidden && ($MG_albums[0]->owner_id /*SEC_hasRights('mediagallery.admin')*/ ) ) ) {
                   $album_jumpbox .= '<option value="' . $this->id . '"' . ($this->id == $selected ? ' selected="selected" ' : '') .'>' . $indent;
                    ...
                    if ( strlen( $tatitle ) > 50 ) {
                        $aTitle = COM_truncate( $this->title, 50 ) . '...';
                    } else {
                        $aTitle = $tatitle;
                    }
    ...
    function albumThumbnail( ) {
        ...
        $F->set_var(array(
            'border_width'          => $newwidth  + (($MG_albums[$this->parent]->album_skin == 'mgShadow') ? 12 : 18),
            'border_height'         => $newheight + (($MG_albums[$this->parent]->album_skin == 'mgShadow') ? 12 : 18),
...
//        $C = new Template($_MG_CONF['template_path']);
        $C = MG_templateInstance( MG_getTemplatePath($this->parent) );
