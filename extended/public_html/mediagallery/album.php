...
function MG_buildAdminbox(&$album, &$root_album, &$T)
{
    ...
    if ($isShowUploadMenu || $isShowAdminMenu) {
        ...
        $admin_box .= '<select name="mode" class="uk-select uk-form-width-medium" onchange="forms[\'adminbox\'].submit()">' . LB;
...
function MG_buildSortbox($album_id, $sortOrder, $page)
{
    ...
    $retval .= $LANG_MG03['sort_by'] . ':&nbsp;'
             . '<select class="uk-select uk-form-width-medium" name="sort" onchange="forms[\'sortbox\'].submit()">' . LB;
