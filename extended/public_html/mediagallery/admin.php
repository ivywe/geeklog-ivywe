...
function MG_navbar($selected='', $album_id)
{
    ...
    $navbar->set_onclick($LANG_MG01['browser_upload'], 'location.href="' . $_MG_CONF['site_url'] . '/admin.php?mode=browser&amp;album_id=' . $album_id . '";');

    if (SEC_hasRights('mediagallery.admin')) {
        $navbar->add_menuitem($LANG_MG01['ftp_media'], $_MG_CONF['site_url'] . '/admin.php?mode=import&amp;album_id=' . $album_id);
        $navbar->set_onclick($LANG_MG01['ftp_media'], 'location.href="' . $_MG_CONF['site_url'] . '/admin.php?mode=import&amp;album_id=' . $album_id . '";');
    }

    $navbar->add_menuitem($LANG_MG01['remote_media'], $_MG_CONF['site_url'] . '/admin.php?mode=remote&amp;album_id=' . $album_id);
    $navbar->set_onclick($LANG_MG01['remote_media'], 'location.href="' . $_MG_CONF['site_url'] . '/admin.php?mode=remote&amp;album_id=' . $album_id . '";');
...
if ($mode === 'edit') {
    ...
    $display = MG_createHTMLDocument($display);
    COM_output($display);
} elseif ($mode == $LANG_MG01['copy'] && !empty($LANG_MG01['copy'])) {
    $album_id = (int) Input::fPost('album_id', -1);
    if ($album_id < 0) {
        MG_invalidRequest();
    }
    ...
    $actionURL = $_MG_CONF['site_url'] . '/album.php?aid=' . $album_id;
    if ($destination == 0) { // deny copy to the root album
        COM_redirect($actionURL);
    }
