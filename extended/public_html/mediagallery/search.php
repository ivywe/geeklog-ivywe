...
// +--------------------------------------------------------------------------+
// | $Id:: search.php 5614 2010-03-19 19:08:33Z mevans0263                   $|
// +--------------------------------------------------------------------------+
...
function MG_searchDisplayThumb( $M, $sortOrder, $id, $page, $force=0 ) {
    ...
    $nFrame = new mgFrame();
    $frame_skin = (!empty($_MG_CONF['search_frame_skin'])) ? $_MG_CONF['search_frame_skin'] : 'mgShadow';
    $nFrame->constructor($frame_skin);
    ...
    } elseif (isset($_MG_CONF['search_tn_size'])) {
    ...
    if ($MG_albums[$M['album_id']]->enable_rating > 0) {
        ...
        $rating_box = MG_getRatingBar($M['album_id'], $M['owner_id'], $M['media_id'], $M['media_votes'], $M['media_rating'], 'sm');
    }
    ...
    $L = MG_templateInstance( MG_getTemplatePath(0) );
//    $skin = 'default';
//    if (!empty($_MG_CONF['search_album_skin'])) {
//        $skin = $_MG_CONF['search_album_skin'];
//    }
//    $L = MG_templateInstance(MG_getTemplatePath_byName($skin));
...
$skin = (!empty($_MG_CONF['search_album_skin'])) ? $_MG_CONF['search_album_skin'] : $MG_albums[0]->skin;
$themeStyle .= MG_getThemePublicJSandCSS($skin); // Quicker than MG_getThemeCSS
$themeStyle .= MG_getThemeCSS($skin);
$title = $LANG_MG00['results'];
