...
//              $retval .= '<p>';
            ...
            if ( $depth == 0 ) {
                $retval .= "<div style=\"margin-left:" . $px . "px;\">"  . '<input type="checkbox" name="gallery[' . $this->id . ']" id="gallery_' . $this->id . '_" value="1" ' . $block . XHTML .'>&nbsp;&nbsp;' . strip_tags(COM_stripslashes($this->title)) . ' (' . $this->media_count . ') </div>' . LB;
            } else {
                if ( $level <= $depth ) {
...
function MG_importSelectAlbums() {
    ...
    $T->set_file('page', 'gl_story_import.thtml');

    $level = 0;
    $album_jumpbox  = '<select id="destination" name="destination">';
