...
// +---------------------------------------------------------------------------+
// | $Id:: install.php 3318 2008-10-06 01:54:13Z mevans0263                   $|
...
$pi_name    = $_MG_CONF['pi_name'];         // Plugin name  Must be 15 chars or less
$pi_version = $_MG_CONF['pi_version'];      // Plugin Version
$gl_version = $_MG_CONF['gl_version'];      // GL Version plugin for
$pi_url     = $_MG_CONF['pi_url'];          // Plugin Homepage
...
if (!SEC_inGroup('Root')) {
    ...
    COM_errorLog("Someone has tried to illegally access the Media Gallery install/uninstall page.  User id: {$_USER['uid']}, Username: {$_USER['username']}, IP: " . $_SERVER['REMOTE_ADDR'],1);
...
function plugin_install_mediagallery()
{
    ...
    DB_query("INSERT INTO {$_TABLES['mg_config']} VALUES ('ad_group_id', '" . $group_id . "')");
...
$display = MG_createHTMLDocument($display, array('what' => 'menu', 'rightblock' => false));
