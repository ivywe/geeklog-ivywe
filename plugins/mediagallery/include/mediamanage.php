...
function MG_imageAdmin( $album_id, $page, $actionURL = '' ) {
    ...
    if ( $_DB_dbms == "mssql" ) {
        $sql = "SELECT *,CAST(media_desc AS TEXT) AS media_desc FROM " .
                ...
                " ORDER BY ma.media_order ASC LIMIT " . $begin . "," . $end;
    } else {
        $sql = "SELECT * FROM " .
                $_TABLES['mg_media_albums'] .
                " as ma INNER JOIN " .
            ...
            for ($j = $x; $j < ($x + 3); $j++) {
                ...
                if ( $img_size != false ) {
                    if ( $img_size[0] > $img_size[1] ) {
                        $ratio = $img_size[0] / 240;
                        $width = 240;
                        $height = round($img_size[1] / $ratio);
                    } else {
                        $ratio = $img_size[1] / 240;
                        $height = 240;
                ...
                if ( $row['media_type'] == 0 ) {
                    ...
                    $dWidth = $disp_size[0] + 12;
                    $dHeight = $disp_size[1] + 12;
                    $T->set_var(array(
