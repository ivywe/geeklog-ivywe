...
function MG_centerblock($where=1, $page=1, $topic ='')
{
    global $_CONF, $_MG_CONF, $MG_albums, $_TABLES, $_USER, $LANG_MG00, $LANG_MG01, $LANG_MG03;

    $pi_name = 'mediagallery';     // Plugin name
    $retval = '';

    if ( $_MG_CONF['gallery_only'] == 1 && $topic == '') {
        return MG_galleryOnly();
    }
    ...
    $result = DB_query("SELECT album_id, cbpage, cbposition FROM {$_TABLES['mg_albums']} WHERE featured='1' AND (cbpage='" . addslashes($sTopic) . "' OR cbpage='all' OR cbpage='allnhp') " . COM_getPermSQL('and') . " LIMIT 1");
